...
def annotation_diff_method(row):
    ...
    if "how did" in q and "change" in q:
        if change_type == "replacement":
            return f"The {field_name} changed from {old_value} to {new_value}."
        if change_type == "deletion":
            return f"The {field_name} value {old_value} was removed."
        if change_type == "insertion":
            return f"The {field_name} value {new_value} was added."
